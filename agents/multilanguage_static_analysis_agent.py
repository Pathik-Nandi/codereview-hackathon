...
class MultiLanguageStaticAnalysisAgent(BaseAgent):
    ...
    def _check_todo_comments(self, code: str, filename: str, line_number: int, issues: List[Issue], language: str):
    ...
    def _check_hardcoded_secrets(self, code: str, filename: str, line_number: int, issues: List[Issue], language: str):
    ...
    def _check_sql_injection(self, code: str, filename: str, line_number: int, issues: List[Issue], language: str):
    ...
    def _check_eval_usage(self, code: str, filename: str, line_number: int, issues: List[Issue], language: str):
    ...
    def _check_java_empty_catch(self, code: str, lines: List[str], current_index: int, 
        ...
        if code.strip() == 'catch' or (code.strip().startswith('catch') and '{' in code):
            ...
            if any(l.strip() == '+}' or l.strip() == '}' for l in next_lines):
                issues.append(Issue(
                    type=IssueType.QUALITY,
                    severity=Severity.HIGH,
    ...
    def _analyze_js_ts_file(self, file, language: str) -> List[Issue]:
        ...
        for line in lines:
            ...
            self._check_js_console_log(code, file.filename, line_number, issues, language)
            self._check_js_equality(code, file.filename, line_number, issues, language)
            self._check_js_var_usage(code, file.filename, line_number, issues, language)
            self._check_js_empty_catch(code, file.filename, line_number, issues, language)
    ...
    def _check_js_console_log(self, code: str, filename: str, line_number: int, issues: List[Issue], language: str):
        """Check for console.log usage in JS/TS code."""
    ...
    def _check_js_equality(self, code: str, filename: str, line_number: int, issues: List[Issue], language: str):
        """Check for non-strict equality operators in JS/TS code."""
        if ' == ' in code or ' != ' in code:
    ...
    def _check_js_var_usage(self, code: str, filename: str, line_number: int, issues: List[Issue], language: str):
        """Check for var usage in JS/TS code."""
    ...
    def _check_js_empty_catch(self, code: str, filename: str, line_number: int, issues: List[Issue], language: str):
        """Check for empty catch blocks in JS/TS code."""
